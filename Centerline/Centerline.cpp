...
//std::cout<<"Try a smaller value of confidenceThreshold or Inverse normal might work !!! "<<std::endl;
    ...
    */
    ...
    while(myDomain.isInside(DGtal::PointVector<3, int>(nextFront))){
    ...
    while(myDomain.isInside(DGtal::PointVector<3, int>(nextBack))){
...
Centerline::computeRawLine(std::vector<Z3i::RealPoint> &rawLine){
    ...
    Image3D imageAcc = getAccumulationImage();
    
    //DGtal::VolWriter< Image3DChar >::exportVol("test.vol", imageAcc, false);

    DGtal::LongvolWriter<Image3D>::exportLongvol("imageAcc.longvol",imageAcc);
    ...
    //trace.info()<<"maxZ"<<maxZ<<std::endl;
    //trace.info()<<"minZ"<<minZ<<std::endl;
    //trace.info()<<"zLength"<<zLength<<std::endl;
    //trace.info()<<"nbSegment"<<nbSegment<<std::endl;
    ...
    //trace.info()<<"radiiThreshold"<<radiiThreshold<<std::endl;
