...
SimpleNormalAccumulator::computeAccumulation(bool retainVertexAsso, bool verbose){
    ...
    for (unsigned int numNorm = 0; numNorm < directions.size(); numNorm++){
        ...
        while((currentPoint - centerPoint).norm()<myRadius){
            if(myDomain.isInside(DGtal::PointVector<3, int>(currentPoint)) && previousPoint != currentPoint){
               
                // retain for each voxel its associated faces contributing to the accumulation
                if(retainVertexAsso){
                    std::vector<unsigned int> v = myAssociationImage(DGtal::PointVector<3, int>(currentPoint));
                    v.push_back(numNorm);        
                    myAssociationImage.setValue(DGtal::PointVector<3, int>(currentPoint), v);
                }

                myAccumulationImage.setValue(DGtal::PointVector<3, int>(currentPoint), myAccumulationImage(DGtal::PointVector<3, int>(currentPoint))+1);

                myRadiusImageAcc.setValue(DGtal::PointVector<3, int>(currentPoint), myRadiusImageAcc(DGtal::PointVector<3, int>(currentPoint)) + (currentPoint - centerPoint).norm());

                previousPoint = currentPoint;

                double dist = (currentPoint - centerPoint).norm(); 

                if( dist < myImageMinRadius(DGtal::PointVector<3, int>(currentPoint) )){
                    myImageMinRadius.setValue(DGtal::PointVector<3, int>(currentPoint), dist);
                }

                if( dist > myImageMaxRadius(DGtal::PointVector<3, int>(currentPoint)) ) {
                    myImageMaxRadius.setValue(DGtal::PointVector<3, int>(currentPoint), dist);
                }

                // update the max of accumulation
                if( myAccumulationImage(DGtal::PointVector<3, int>(currentPoint)) > myMaxAccumulation ) {
                    myMaxAccumulation = myAccumulationImage(DGtal::PointVector<3, int>(currentPoint));
                    myMaxAccumulationPoint = DGtal::PointVector<3, int>(currentPoint);
                }
...
SimpleNormalAccumulator::computeConfidence(bool updateVertexAsso){
    ...
    for (unsigned int numNorm = 0; numNorm< directions.size(); numNorm++){
        ...
        while((currentPoint - centerPoint).norm()<myRadius){
            if(myDomain.isInside(DGtal::PointVector<3, int>(currentPoint)) && previousPoint != currentPoint){        
                unsigned int valAcc = myAccumulationImage(DGtal::PointVector<3, int>(currentPoint));
                if( valAcc > aMaxAcc){
                    aMaxAcc = valAcc;
        ...
        if(aPtMaxAcc != centerPoint){
            scoreConfidance.setValue(DGtal::PointVector<3, int>(aPtMaxAcc), scoreConfidance(DGtal::PointVector<3, int>(aPtMaxAcc)) + 1);
            if (scoreConfidance(DGtal::PointVector<3, int>(aPtMaxAcc)) > maxAccVerified) {
                maxAccVerified = scoreConfidance(DGtal::PointVector<3, int>(aPtMaxAcc));
            }
            if (updateVertexAsso){
                std::vector<unsigned int> v = myAssociationImage(DGtal::PointVector<3, int>(aPtMaxAcc));
                v.push_back(numNorm);        
                myAssociationImage.setValue(DGtal::PointVector<3, int>(aPtMaxAcc), v);
            }
        }
